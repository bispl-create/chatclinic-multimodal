...
@registry.register_model("vllama_stage2_opt")
class vllamastage2opt(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.tokenizer = open_clip.get_tokenizer('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224') #self.init_tokenizer()
        ...
        # self.visual_encoder = self.init_DINO_encoder(vit_model, vit_path[0], patch_size=16, evaluate=self.evaluate)
        ...
        ###USING Z-Former
        self.z_path = z_path
        self.Zformer = self.init_Zformer(big_bird=big_bird, model_path=self.z_path)
        ...
        if big_bird:
            ...
            self.Zformer.embeddings.position_ids.requires_grad = False
        else:
        ...
        #print('PRE self.llama_tokenizer.pad_token_id', self.llama_tokenizer.pad_token_id, self.llama_tokenizer.eos_token_id, self.llama_tokenizer.pad_token, self.llama_tokenizer.eos_token)
        ...
        if self.low_resource:
            ...
            #self.llama_model.to(f'cuda:{dist.get_rank()}')
        ...
        #self.llama_model.model.model.decoder.embed_tokens.bias.requires_grad = False
    ...
    def encode_img(self, image, attn_mask):
        ...
        #     image_embeds, _, _ = self.visual_encoder(image_reshaped, self.tokenizer("Empty text").to(self.llama_model_device))
        ...
        assert inputs_llama.requires_grad, "img_embeds is detached from computation graph!"
    ...
    def prompt_vqa_wrap(self, img_embeds, atts_img, prompt):
        ...
        for idx, question in enumerate(prompt):
            ...
            p_after_token_embeds = self.llama_model.model.model.decoder.embed_tokens(encoded_question)
    ...
    def prompt_conv_vqa_wrap(self, img_embeds, batch_conversations):
        """
        Prepares multi-turn VQA inputs for training.

        - Embeds the **system prompt** and ensures it is always visible.
        - Embeds **questions (RADIOLOGIST:)** but ensures they do not contribute to loss.
        - Embeds **answers (ASSISTANT:)** and ensures they contribute to loss.
        - Ensures **attention_mask is uniform (all ones)** for causal decoding.
        - Ensures **labels (targets) properly align with assistant responses**.

        Returns:
            - `text_embeds`: The input text embeddings for the LLM.
            - `attention_mask`: The attention mask (all ones).
            - `targets`: The labels ensuring only `ASSISTANT:` tokens contribute to loss.
        """
        #batch_size = img_embeds.shape[0]
        text_embeds_list = []
        targets_list = []
        ...
        for conv in batch_conversations:
            turns = conv.split("</s>")  # Split conversation into turns
            encoded_turns = []
            target_masks = []
            q_embeds = None  # Track the last question embeddings

            ## STEP 1: Embed System Prompt (First Line) ##
            system_prompt = turns[0].strip()  # Extract system prompt
            system_prompt_tokens = self.llama_tokenizer(
                system_prompt, return_tensors="pt", add_special_tokens=False
            ).to(img_embeds.device)

            system_prompt_embeds = self.llama_model.model.model.decoder.embed_tokens(system_prompt_tokens.input_ids)
            system_prompt_targets = torch.full_like(system_prompt_tokens.input_ids, -100)  # No loss on system prompt
            #target_masks.append(system_prompt_targets)
            ## STEP 2: Embed Multi-Turn Q&A ##
            for idx, turn in enumerate(turns[1:]):  # Skip system prompt
                turn = turn.strip()

                if "RADIOLOGIST:" in turn and "<ImageHere>" in turn:
                    #Insert Image Embeddings in <ImageHere>
                    p_before, p_after = turn.split('<ImageHere>')
                    image_before = self.llama_tokenizer(p_before, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                    image_after = self.llama_tokenizer(p_after, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                    image_before_embeds = self.llama_model.model.model.decoder.embed_tokens(image_before.input_ids)
                    image_after_embeds = self.llama_model.model.model.decoder.embed_tokens(image_after.input_ids)
                    q_embeds = torch.cat([image_before_embeds, img_embeds, image_after_embeds], dim=1)
                    q_targets = torch.full_like(q_embeds, -100)

                elif "RADIOLOGIST:" in turn and "<ImageHere>" not in turn:
                    # Store question embedding but DO NOT append yet
                    q_tokens = self.llama_tokenizer(turn, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                    q_embeds = self.llama_model.model.model.decoder.embed_tokens(q_tokens.input_ids)
                    q_targets = torch.full_like(q_tokens.input_ids, -100)  # No loss on questions

                elif "ASSISTANT:" in turn and q_embeds is not None:
                    # Tokenize the answer
                    a_tokens = self.llama_tokenizer(turn, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                    a_embeds = self.llama_model.model.model.decoder.embed_tokens(a_tokens.input_ids)
                    a_targets = a_tokens.input_ids.masked_fill(a_tokens.input_ids == self.llama_tokenizer.pad_token_id, -100)  # Train only on answers

                    # Concatenate Q&A embeddings
                    turn_embeds = torch.cat([q_embeds, a_embeds], dim=1)
                    turn_targets = torch.cat([q_targets, a_targets], dim=1)
                    #print('turn_targets.shape', turn_targets.shape)
                    encoded_turns.append(turn_embeds)
                    #target_masks.append(q_targets)
                    target_masks.append(turn_targets)

                    # Reset question embedding tracker
                    q_embeds = None

            if not encoded_turns:
                continue  # Skip conversations with no valid Q&A pairs

            ## STEP 3: Merge System Prompt + Q&A Turns ##
            print('system_prompt_embeds.shape', system_prompt_embeds.shape)
            print('system_prompt_target.shape', system_prompt_targets.shape)
            #print('target_masks.shape', target_masks.shape)
            text_embeds = torch.cat([system_prompt_embeds] + encoded_turns, dim=1)
            targets = torch.cat([system_prompt_targets] + target_masks, dim=1)
            print('text_embeds.shape', text_embeds.shape)
            print('targets.shape', targets.shape)
            max_sequence_length = max(max_sequence_length, text_embeds.shape[1])
        ...
        def pad_tensor_list(tensor_list, pad_value=0):
            ...
            for tensor_id in tensor_list:
                pad_size = max_sequence_length - tensor_id.shape[1]
                if pad_value == 0:
                    padded_tensor_id = torch.cat([tensor_id, torch.full((tensor_id.shape[0], pad_size, tensor_id.shape[2]), pad_value, device=tensor_id.device)], dim=1)
                else:
                    print('tensor_id.shape', tensor_id.shape, pad_size, pad_value)
                    padded_tensor_id = torch.cat([tensor_id, torch.full((tensor_id.shape[0], pad_size), pad_value, device=tensor_id.device)], dim=1)
                padded_tensors.append(padded_tensor_id)
            return torch.cat(padded_tensors, dim=0)
        # Stack all batches

        text_embeds = pad_tensor_list(text_embeds_list, pad_value=0)
        targets = pad_tensor_list(targets_list, pad_value=-100)

        ## STEP 4: Create Attention Mask (Uniform Ones) ##
        # attention_mask = torch.ones_like(targets, dtype=torch.long).to(img_embeds.device)  # Every token is visible

        return text_embeds, targets
    ...
    def forward(self, samples):
        ...
        #text_s = samples[2]#.to(self.llama_model_device)
        modality = samples[2]
        attn_mask = samples[3].to(self.llama_model_device)
        #bs, ds, c, h, w = image.size()

        img_embeds = self.encode_img(image, attn_mask)
        atts_img = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(self.llama_model_device)
        #atts_img = atts_img.to(self.llama_model_device)
        #llm_loss = 0
        #print('Encode_img fine')
        ###MODIFY THIS PART FOR VQA GENERATION###
        if modality[0] == "brainCTConvVQA":  # VQA dataset
            print('VQA Batch')
            #report = ""
            vqa_prompt = "A chat between a radiologist and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the radiologist's questions. </s> {}"
            batch_conversation = [vqa_prompt.format(conversation) for conversation in text_f]
            #print('batch_conversation', batch_conversation)
            img_text_embeds, targets = self.prompt_conv_vqa_wrap(img_embeds, batch_conversation)

            batch_size = img_embeds.shape[0]
            bos = torch.ones([batch_size, 1], dtype=torch.long, device=self.llama_model_device) * self.llama_tokenizer.bos_token_id
            bos_embeds = self.llama_model.model.model.decoder.embed_tokens(bos)

            #print('targets.shape', targets.shape)
            #print('img_text_embesd.shape', img_text_embeds.shape)
            targets = torch.cat([torch.full((batch_size, 1), -100, dtype=torch.long, device=self.llama_model_device), targets], dim=1)

            # Ensure `targets` and `inputs_embeds` have the same length
            assert targets.shape[1] == img_text_embeds.shape[1] + 1, f"Target length {targets.shape[1]} does not match input length {img_text_embeds.shape[1] + 1}"

            attention_mask = torch.ones_like(targets, dtype=torch.long).to(self.llama_model_device)  # Every token is visible.

            inputs_embeds = torch.cat([bos_embeds, img_text_embeds], dim=1)

            ## Step 4: Pass to LLaMA Model ##
            outputs = self.llama_model(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
                return_dict=True,
                labels=targets  # Ensures correct loss alignment
            )

            llm_loss = outputs.loss
            print('loss:', llm_loss.item())

            return {"loss": llm_loss}

        elif modality[0] == 'brainCT':
            prompt = random.choice(self.prompt_dict['brainCT'])
            ...
            to_regress_embeds = self.llama_model.model.model.decoder.embed_tokens(to_regress_tokens.input_ids)
    ...
    def from_config(cls, cfg):
        ...
        big_bird = cfg.get("big_bird", False)
        evaluate = cfg.get("evaluate", False)
        ve_type = cfg.get("ve_type", None)
