...
@registry.register_model("vllama_stage1_opt")
class vllamastage1opt(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.llama_model.model.decoder.embed_tokens.weight.requires_grad = False
        #self.llama_model.model.decoder.embed_tokens.bias.requires_grad = False
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt):
        if prompt:
            ...
            p_before_embeds = self.llama_model.model.decoder.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1)
            p_after_embeds = self.llama_model.model.decoder.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def prompt_vqa_wrap(self, img_embeds, atts_img, prompt):
        ...
        for idx, question in enumerate(prompt):
            ...
            p_after_token_embeds = self.llama_model.model.decoder.embed_tokens(encoded_question)
    ...
    def forward(self, samples):
        ...
        bos_embeds = self.llama_model.model.decoder.embed_tokens(bos)
        #bos_embeds = self.llama_model.embed_tokens(bos)
        atts_bos = atts_img[:, :1]

        to_regress_embeds = self.llama_model.model.decoder.embed_tokens(to_regress_tokens.input_ids)
    ...
    def from_config(cls, cfg):
        ...
        ve_type = cfg.get("ve_type", 'BioMedCLIP')
