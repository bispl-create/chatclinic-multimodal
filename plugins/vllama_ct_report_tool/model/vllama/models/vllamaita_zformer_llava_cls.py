...
@registry.register_model("vllamaita_zformer_llava_cls")
class vllamaItaZformerLLaVACls(Blip2Base):
    """
    ...
    def __init__(
        ...
        if freeze_vit:
            ...
            #     param.requires_grad = False
            for idx, layer in enumerate(self.visual_encoder.blocks):
                if idx == (len(self.visual_encoder.blocks)-1):
    ...
    def vis_encode(self, image):
        ...
        with self.maybe_autocast():
            ...
            for idx in range(image.shape[1]):
                ...
                _ = self.visual_encoder(slice_image).to(self.llama_model_device)

                slice_embeds = self.extract_embeddings_from_heads().to(self.llama_model_device)
                # Clear the raw_attention_maps to free memory and avoid data leak between forwards
                self.raw_attention_maps = []
    ...
    def encode_img(self, image, attn_mask, modality):
        ...
        with self.maybe_autocast():
            ...
            image_embeds = z_output.last_hidden_state[:,:,:]
