...
@registry.register_model("vllama_stage1_3d")
class vllamastage1threedim(Blip2Base):
    """
    ...
    def __init__(
        ...
        elif self.ve_type =='3D':
            self.visual_encoder = self.init_3D_encoder(model_path=self.z_path, num_layers=4, num_heads=8)
            self.visual_encoder.to(device)
        else:
        ...
        self.perceiver = PerceiverResampler(dim=1024, depth=depth, num_latents=num_query_token)
        ...
        #self.llama_model.model.model.model.embed_tokens.bias.requires_grad = False
        ...
        self.perc_proj_chz = nn.Linear(512, 1024).to(self.llama_model_device)
        self.llama_proj_chz = nn.Linear(1024, self.llama_model.config.hidden_size).to(self.llama_model_device)
    ...
    def encode_img(self, image, attn_mask, modality):
        ...
        if self.ve_type == 'BioMedCLIP':
            ...
            #image_reshaped = image_embeds[:,:,:].view(1, 400, 20, 512)
            #recon_inputs = image_reshaped.mean(dim=2)
            #recon_inputs = image_embeds
    ...
    def forward(self, samples):
        ...
        if modality[0][:12] == 'CtRATEGround':
            ...
            prompt = random.choice(self.prompt_dict['rectalMRI'])
            img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img, prompt)
            text = [t + self.end_sym for t in text_s]
            print('prompt/report:', prompt, text)
