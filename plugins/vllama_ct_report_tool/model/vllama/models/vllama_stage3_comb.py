...
@registry.register_model("vllama_stage3_comb")
class vllamastage3Comb(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.prompt_dict["CTRATEComb"] = []
        for elem in self.prompt_list:
            # split_elem = elem.split(" ")
            # if "normal" in split_elem:
            self.prompt_dict["CTRATEComb"].append(elem)
            # else:
    ...
    def prompt_wrap_comb(self, img_embeds, atts_img, prompt):
        if prompt:
            ...
            p_before_img_embeds = self.llama_model.model.embed_tokens(p_before_img_tokens.input_ids).expand(batch_size, -1, -1)
            p_after_img_embeds = self.llama_model.model.embed_tokens(p_after_img_tokens.input_ids).expand(batch_size, -1, -1)
            p_after_imp_embeds = self.llama_model.model.embed_tokens(p_after_imp_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def forward(self, samples):
        ...
        text_s = samples[1][0]#.to(self.llama_model_device)
        text_imp = samples[1][1]
        modality = samples[2]
        ...
        print('prompt: ', prompt)
        img_embeds, atts_img, report_instruction_embeds, atts_report_instruction = self.prompt_wrap_comb(img_embeds, atts_img, prompt)
        text_report = [t + self.end_sym for t in text_s]
        print('text_imp', type(text_imp), text_imp, samples[3])
        print('text_imp[0]', type(text_imp[0]))
        text_imp = [str(t) + self.end_sym for t in text_imp]
        # if modality[0] == "CTRATEComb":
        #     #print('modality0', modality, modality[0])
        #     prompt = random.choice(self.prompt_dict[modality[0]])
        #     img_embeds, atts_img = self.prompt_wrap_comb(img_embeds, atts_img, prompt)
        #     text = [t + self.end_sym for t in text_s]

        # elif modality[0] == "CTRATEComb":
        #     #print('modality0', modality, modality[0])
        #     prompt = random.choice(self.prompt_dict[modality[0]])
        #     img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img, prompt)
        #     text = [t + self.end_sym for t in text_s]

        # elif modality[0] == "CTRATEImp":
        #     #print('modality0', modality, modality[0])
        #     prompt = random.choice(self.prompt_dict[modality[0]])
        #     img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img, prompt)
        #     text = [t + self.end_sym for t in text_s]

        # elif modality[0] == "CtRATEVQA":  # VQA dataset
        #     print('VQA Batch')
        #     vqa_prompt = '<Img><ImageHere></Img> Diagnose this 3D chest CT scan with findings given the following question:'
        #     question = text_s[0]
        #     print('question', question, len(question))
        #     question_prompt = [vqa_prompt+" "+q for q in question]
        #     img_embeds, atts_img = self.prompt_vqa_wrap(img_embeds, atts_img, question_prompt)
        ...
        to_regress_report_tokens = self.llama_tokenizer(
            text_report,
            return_tensors="pt",
            padding="longest",
            truncation=True,
        ...
        #print('to_shape', to_regress_tokens.input_ids.shape)
        targets_pre_report = to_regress_report_tokens.input_ids.masked_fill(
            to_regress_report_tokens.input_ids == self.llama_tokenizer.pad_token_id, -100
        )
        print('targets_pre_report:' , targets_pre_report)
        to_regress_imp_tokens = self.llama_tokenizer(
            text_imp,
            return_tensors="pt",
            padding="longest",
            truncation=True,
        ...
        #print('to_shape', to_regress_tokens.input_ids.shape)
        targets_pre_imp = to_regress_imp_tokens.input_ids.masked_fill(
            to_regress_imp_tokens.input_ids == self.llama_tokenizer.pad_token_id, -100
        )
        print('targets_pre_imp:' , targets_pre_imp) # Expect to have -100 at the very end and the rest as 1.
        #print('repeat size', atts_img.shape[0]//targets_pre.shape[0])
        print('PRE targets_pre_report.shape, targets_pre_impression.shape', targets_pre_report.shape, targets_pre_imp.shape)
        targets_report = targets_pre_report.repeat(atts_img.shape[0]//targets_pre_report.shape[0], 1)
        targets_impression = targets_pre_imp.repeat(atts_img.shape[0]//targets_pre_imp.shape[0], 1)
        print('POST targets_report.shape, targets_impression.shape', targets_report.shape, targets_impression.shape)
        #print('targets new', targets.size())
        #print('atts_img shape', atts_img.shape)
        empty_targets_pre_impression = (
            torch.ones([atts_img.shape[0], atts_img.shape[1]+1],
                    dtype=torch.long).to(self.llama_model_device).fill_(-100)  # plus one for bos
        )

        empty_targets_post_impression = (
            torch.ones([atts_report_instruction.shape[0], atts_report_instruction.shape[1]],
                    dtype=torch.long).to(self.llama_model_device).fill_(-100)  # plus one for bos
        )


        targets = torch.cat([empty_targets_pre_impression, targets_impression, empty_targets_post_impression, targets_report], dim=1)
        print('targets device', targets.device)
        print('targets, ', targets)
        batch_size = img_embeds.shape[0]
        bos = torch.ones([batch_size, 1],
                        dtype=to_regress_report_tokens.input_ids.dtype,
                        device=to_regress_report_tokens.input_ids.device) * self.llama_tokenizer.bos_token_id
        bos_embeds = self.llama_model.model.embed_tokens(bos)
        #bos_embeds = self.llama_model.embed_tokens(bos)
        atts_bos = atts_img[:, :1]
        print('attn_bos,', atts_bos)


        to_regress_report_embeds = self.llama_model.model.embed_tokens(to_regress_report_tokens.input_ids)
        print('PRE to_regress_report_embeds.shape,', to_regress_report_embeds.shape)
        #to_regress_embeds = self.llama_model.embed_tokens(to_regress_tokens.input_ids)
        to_regress_report_embeds = to_regress_report_embeds.repeat(img_embeds.shape[0]//to_regress_report_embeds.shape[0], 1, 1)
        print('POST to_regress_report_embeds.shape,', to_regress_report_embeds.shape)


        to_regress_impression_embeds = self.llama_model.model.embed_tokens(to_regress_imp_tokens.input_ids)
        print('PRE to_regress_impression_embeds.shape,', to_regress_impression_embeds.shape)
        #to_regress_embeds = self.llama_model.embed_tokens(to_regress_tokens.input_ids)
        to_regress_impression_embeds = to_regress_impression_embeds.repeat(report_instruction_embeds.shape[0]//to_regress_impression_embeds.shape[0], 1, 1)
        print('POST to_regress_impression_embeds.shape,', to_regress_impression_embeds.shape)

        #img_embeds = img_embeds / 10
        inputs_embeds = torch.cat([bos_embeds, img_embeds, to_regress_impression_embeds, report_instruction_embeds, to_regress_report_embeds], dim=1)
        print('PRE to_regress_imp_tokens.attention_mask.shape,', to_regress_imp_tokens.attention_mask.shape)
        to_regress_imp_tokens.attention_mask = to_regress_imp_tokens.attention_mask.repeat(atts_img.shape[0]//to_regress_imp_tokens.attention_mask.shape[0], 1)
        print('Post to_regress_imp_tokens.attention_mask.shape,', to_regress_imp_tokens.attention_mask.shape)
        print('PRE to_regress_report_tokens.attention_mask.shape,', to_regress_report_tokens.attention_mask.shape)
        to_regress_report_tokens.attention_mask = to_regress_report_tokens.attention_mask.repeat(atts_report_instruction.shape[0]//to_regress_report_tokens.attention_mask.shape[0], 1)
        print('Post to_regress_report_tokens.attention_mask.shape,', to_regress_report_tokens.attention_mask.shape)
        attention_mask = torch.cat([atts_bos, atts_img, to_regress_imp_tokens.attention_mask, atts_report_instruction, to_regress_report_tokens.attention_mask], dim=1)
        print('FINAL attention_mask', attention_mask) ### ALL 1s but 0 for self.end_sym after impression & report
