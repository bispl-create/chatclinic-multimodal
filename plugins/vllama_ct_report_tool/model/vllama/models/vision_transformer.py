...
from vllama.models.utils import trunc_normal_
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(x.shape)
        x = self.proj(x)#.flatten(2,-1).transpose(1,2)
        #print(x.shape)
        x = x.flatten(2,-1)
        #print(x.shape)
        x = x.transpose(1, 2)
...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        #patch_pos_embed = patch_pos_embed.unsqueeze(dim=2)
        #print('patch_pos_embed', patch_pos_embed.shape)
        #patch_pos_embed = patch_pos_embed.expand(-1,-1, d0, -1, -1)
        #print('patch_pos_embed', patch_pos_embed.shape)
    ...
    def prepare_tokens(self, x):
        #print('Prepare_tokens', x.shape)
        #B, nc, d, w, h = x.shape
        B, nc, w, h = x.shape
        #print('prerp for pos encoding',x.shape)
