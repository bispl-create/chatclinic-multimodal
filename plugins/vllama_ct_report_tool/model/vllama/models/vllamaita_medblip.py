...
@registry.register_model("vllamaita_medblip")
class vllamaItaMedBLIP(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder_for_medblip(
    ...
    def encode_img(self, image, modality):
        ...
        with self.maybe_autocast():
            ...
            #     image_embeds = self.visual_encoder(image).to(self.llama_model_device)
            #     #print('bmri slice_embeds.shape', slice_embeds.shape)

            # elif modality[0] == "rectalMRI":
            #     slice_embeds = self.visual_encoder(image).to(self.llama_model_device)
            #print(image.shape)
            image = image.permute(0,2,1,3,4)
            image_embeds = self.ln_vision(self.visual_encoder(image)).to(self.llama_model_device)
    ...
    def from_config(cls, cfg):
        vit_model = cfg.get("vit_model", "eva_clip_g") #"ViT-B/32" #"eva_clip_g" 'vit_base'
