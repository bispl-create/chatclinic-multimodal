...
@registry.register_model("vllama_mim")
class vllamaItaZformerMim(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.transform = pth_transforms.Normalize((0.2335, 0.2335, 0.2335), (0.222, 0.222, 0.222)) #pth_transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        ...
        self.Zformer = self.init_Zformer(big_bird=self.big_bird, model_path=self.z_path)
        self.Zformer.to(f'cuda:{dist.get_rank()}')
        self.llama_model_device = self.Zformer.device
        ###
        if self.big_bird:
    ...
    def mask_embeddings(self, image_embeds, mask_prob):
    ...
    def encode_img(self, image, attn_mask):
        ...
        #print('Device', device)
        #image.to(self.llama_model_device)
        ...
        #     slice_embeds = self.visual_encoder(slice_image).pooler_output#.to(self.llama_model_device)
        #     slice_embeds = slice_embeds.unsqueeze(dim=1).detach()
        #     slice_embeds_list.append(slice_embeds)
        #     del slice_image
        ...
        image_embeds, original_embeds, mask = self.mask_embeddings(image_embeds, self.mask_prob)
        ...
        '''
        del slice_embeds_list
        torch.cuda.empty_cache()
        return image_embeds, original_embeds, recon_embeds, mask
    ...
    def forward(self, samples):
        ...
        img_embeds, original_embeds, recon_embeds, mask = self.encode_img(image, attn_mask)

        mae_loss = masked_mae_loss(original_embeds, recon_embeds, mask)

        #print('loss_mae', mae_loss.item())
        del image, attn_mask, original_embeds, recon_embeds, mask
        torch.cuda.empty_cache()
