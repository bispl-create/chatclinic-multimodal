...
@registry.register_model("vllama_stage3")
class vllamastage3(Blip2Base):
    ...
    def prompt_wrap_comb(self, img_embeds, atts_img, prompt):
        if prompt:
            batch_size = img_embeds.shape[0]
            p_before, p_after = prompt[0].split('<ImageHere>')
            p_before_tokens = self.llama_tokenizer(
                p_before, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
            p_after_tokens = self.llama_tokenizer(
                p_after, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
    ...
    def forward(self, samples):
        ...
        if modality[0] == "CTRATE":
            ...
            question = text_s[0]
            print('question', question, len(question))
            question_prompt = [prompt_template.format(vqa_prompt)+" "+q for q in question]
            img_embeds, atts_img = self.prompt_vqa_wrap(img_embeds, atts_img, question_prompt)
            text = [t + self.end_sym for t in text_s[1]]
        ...
        #print('inputs_emebds.device, attention_mask', inputs_embeds.device, attention_mask)
