...
@registry.register_model("vllama_stage2_falcon")
class vllamastage2Falcon(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.llama_tokenizer = AutoTokenizer.from_pretrained(llama_model) #LlamaTokenizer.from_pretrained(llama_model, use_fast=False)
        ...
        if lora_r > 0:
            ...
            print('LLaMA to be finetuned')
        #print(self.llama_model)
        #print(self.llama_model.model)
        self.llama_model.transformer.word_embeddings.weight.requires_grad = False
        #self.llama_model.transformer.word_embeddings.bias.requires_grad = False
        ...
        self.perc_proj_chz = nn.Linear(768, self.llama_model.config.hidden_size).to(self.llama_model_device)
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt):
        if prompt:
            ...
            p_before_embeds = self.llama_model.transformer.word_embeddings(p_before_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def prompt_vqa_wrap(self, img_embeds, atts_img, prompt):
        ...
        for idx, question in enumerate(prompt):
            ...
            p_after_token_embeds = self.llama_model.transformer.word_embeddings(encoded_question)
    ...
    def forward(self, samples):
        ...
        modality = samples[2]#.to(self.llama_model_device)
