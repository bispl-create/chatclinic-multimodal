...
@registry.register_model("vllama_stage2_opt_plusvqa")
class vllamastage2optvqa(Blip2Base):
    """
    ...
    def __init__(
        ...
        #self.llama_model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen2.5-3B-Instruct")
        ...
        # Check and set BOS token
        self.llama_tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-3B-Instruct", trust_remote_code=True)

        if self.llama_tokenizer.bos_token is None:
            self.llama_tokenizer.bos_token = "<|im_start|>"
            self.llama_tokenizer.bos_token_id = self.llama_tokenizer.convert_tokens_to_ids("<|im_start|>")

        # Also check EOS if used elsewhere
        if self.llama_tokenizer.eos_token is None:
            self.llama_tokenizer.eos_token = "<|im_end|>"
            self.llama_tokenizer.eos_token_id = self.llama_tokenizer.convert_tokens_to_ids("<|im_end|>")
        ...
        if self.low_resource:
            ...
            model_name,
            load_in_8bit=True,
            torch_dtype=torch.bfloat16,  # or float32 if needed
            device_map={"": llm_device},  # use appropriate device
            trust_remote_code=True)
            # quant_config = BitsAndBytesConfig(
            #     load_in_4bit=True,                      # Use 4-bit quantization
            #     bnb_4bit_compute_dtype=torch.bfloat16,  # Keep compute in bfloat16
            #     bnb_4bit_use_double_quant=True,         # Double quantization (improves compression)
            #     bnb_4bit_quant_type="nf4"               # Or use "fp4" if desired
            # )

            # self.llama_model = AutoModelForCausalLM.from_pretrained(
            #     model_name,
            #     quantization_config=quant_config,
            #     device_map={"": llm_device},  # You can use if preferred
            #     trust_remote_code=True
            # )
            ...
            # self.llama_model = OPTForCausalLM.from_pretrained(
            #     llama_model,
            #     torch_dtype=torch.float32,  # or float32 if needed
            #     device_map={'': llm_device}
            # )
            # self.llama_model = AutoModelForCausalLM.from_pretrained(
            #     llama_model,
        ...
        if lora_r > 0:
            ...
            for name, param in self.llama_model.named_parameters():
                param.requires_grad = False
            print('Trainable param names after freezing', [name for name, param in self.llama_model.named_parameters() if param.requires_grad])
            print('LLaMA Frozen')
    ...
    def encode_img(self, image, attn_mask, modality):
        ...
        for idx in range(image.shape[1]):
            ...
            slice_image_min = slice_image.min()
            slice_image_max = slice_image.max()

            if slice_image_min != slice_image_max:
                #slice_image = self.transform(slice_image)
                slice_image[slice_image > 180] = 180
                slice_image[slice_image < -20] = -20
                slice_image = (slice_image + 20) / 200
            else:
        ...
        #recon_embeds = torch.cat(slice_embeds_list, dim=1)
        #print('recon_embeds.shape', recon_embeds)
        ...
        #print('recon_embeds.shape', recon_embeds.shape)
        recon_embeds = self.perc_proj_chz(image_embeds)
    ...
    def prompt_conv_vqa_wrap(self, img_embeds, atts_img, batch_conversation):
        p_after_embeds = torch.tensor([]).to(img_embeds.device)
        batch_size = img_embeds.shape[0]
        max_sequence_length = max(len(self.llama_tokenizer.encode(question, add_special_tokens=False)) for question in batch_conversation)
        p_after_embeds_list = []
        for idx, conversation in enumerate(batch_conversation):
            #print('conversation', conversation)
            _, p_after = conversation.split('<ImageHere>')
            if idx == len(batch_conversation)-1:
                p_before, p_after = conversation.split('<ImageHere>')
            encoded_question = self.llama_tokenizer.encode(
                p_after,
                return_tensors="pt",
                add_special_tokens=False,
    ...
    def forward(self, samples):
        ...
        if modality[0] == "brainCTConvVQA":  # VQA dataset
            query, answer = samples[1] #.to(self.llama_model_device)
            #print('VQA Batch')
            #report = ""
            #vqa_prompt = self.prompt_template
            batch_conversation = [self.prompt_template.format(conversation) for conversation in query]
            #print('query', batch_conversation)
            img_embeds, atts_img = self.prompt_conv_vqa_wrap(img_embeds, atts_img, batch_conversation)
            text = [t + self.end_sym for t in answer]
            #print('answer', text)
            #print('prompt/report:', batch_conversation, text)
