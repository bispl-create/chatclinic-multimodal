...
@registry.register_model("vllamaita_sam_zqformer_prealign")
class vllamaItaSamZQformerPreAlign(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.visual_encoder = self.init_SAM_encoder(vit_model, vit_path[0])
        ...
        self.conv1 = nn.Conv2d(256, 128, kernel_size=1)  # Reducing channels: [B, L, 256, 64, 64] -> [B, L, 128, 64, 64]

        # Adaptive pooling
        self.adaptive_pool = nn.AdaptiveAvgPool2d((8, 8))  # Reducing spatial dimensions: [B, L, 128, 64, 64] -> [B, L, 128, 8, 8]

        # Final flattening layer to get [B, L, 768]
        self.flatten = nn.Flatten(start_dim=1)  # [B, L, 128 * 8 * 8] = [B, L, 8192]

        # Another linear layer to map to 768
        self.fc = nn.Linear(128 * 8 * 8, 768)  # [B, L, 8192] -> [B, L, 768]
        ...
        self.qform_proj_chz = nn.Linear(768, 1408).to(self.llama_model_device) #self.visual_encoder.num_features
    ...
    def encode_img(self, image, attn_mask):
        ...
        with self.maybe_autocast():
            ...
            for idx in range(image.shape[1]):
                ...
                slice_embeds = self.visual_encoder.image_encoder(slice_image).to(self.llama_model_device)
            ...
            #print('image_embeds.shape', image_embeds.shape)
            _, _, C, H, W = image_embeds.shape
            image_embeds = image_embeds.view(-1, C, H, W)  # [B*L, 256, 64, 64]

            image_embeds = self.conv1(image_embeds)  # [B*L, 128, 64, 64]
            image_embeds = self.adaptive_pool(image_embeds)  # [B*L, 128, 8, 8]
            image_embeds = self.flatten(image_embeds)  # [B*L, 8192]
            #print('image_embeds.shape', image_embeds.shape)
            image_embeds = self.fc(image_embeds)  # [B*L, 768]

            # Reshape back to original batch and sequence dimension
            image_embeds = image_embeds.view(image.shape[0], image.shape[1], -1)
            ...
            #print('Zformer input image_embeds.shape', image_embeds.shape)
    ...
    def encode_img_m(self, image, attn_mask):
        #device = self.llama_model_device
        #print('Device', device)
    ...
    def forward(self, samples):
        ...
        #print('modality')
        query_output = self.encode_img(image, attn_mask)
        ...
        with torch.no_grad():
            ...
            query_output_m = self.encode_img_m(image, attn_mask)
