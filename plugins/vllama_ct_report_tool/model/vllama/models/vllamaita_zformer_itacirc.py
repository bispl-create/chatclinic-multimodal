...
class vllamaItaZformer(Blip2Base):
    ...
    def __init__(
        ...
        print('numfeatures: ', self.visual_encoder_two.num_features)
        self.qform_proj_chz = nn.Linear(self.visual_encoder_two.num_features, 1408).to(self.llama_model_device) #self.visual_encoder.num_features
        ...
        self.itm_head = nn.Linear(self.Qformer.bert.config.hidden_size, 2)
        #self.qform_proj_chz_m.requires_grad = False
        #self.llama_proj_chz_m.requires_grad = False
    ...
    def encode_img(self, image, modality):
        ...
        with self.maybe_autocast():
            ...
            #print('image_embeds after to q-former projection', image_embeds.shape)
    ...
    def forward(self, samples):
        ...
        img_embeds, atts_img, image_embeds, image_atts, query_output = self.encode_img(image, modality)
        ...
        if modality[0] == "brainMRIVQA":  # VQA dataset
            ...
            # query_idx = query_idx.view(-1, 1).unsqueeze(-1)
            # query_idx = query_idx.expand(-1, -1, query_output.size(2))
            # query_output_selected = torch.gather(query_output, 1, query_idx)

            # img_feat = F.normalize(query_output_selected.squeeze(1), dim=-1)
            # txt_feat = F.normalize(txt_embeds, dim=-1)

            # with torch.no_grad():
            #     self._momentum_update()
            #     query_output_m = self.encode_img_m(image, modality)
            #     query_output_selected_m = torch.gather(query_output_m, 1, query_idx)
            #     image_feat_m = F.normalize(query_output_selected_m.squeeze(1), dim=-1)
            ...
            #     query_output_selected_m = torch.gather(query_output_m, 1, query_idx)
            #     image_feat_m = F.normalize(query_output_selected_m.squeeze(1), dim=-1)
                #print('image_feat_m.shape', image_feat_m.shape)
            ...
            # img_feat = F.normalize(query_output[:,0,:], dim=-1)
            # txt_feat = F.normalize(txt_embeds, dim=-1)
            # with torch.no_grad():
            #     self._momentum_update()
            #     query_output_m = self.encode_img_m(image, modality)
            #     #image_feat_m = F.normalize(query_output_m.mean(dim=1), dim=-1)
            #     image_feat_m = F.normalize(query_output_m[:,0,:], dim=-1)
                #print('image_feat_m.shape', image_feat_m.shape)
        ...
        #print('query_output.shape', query_output.shape)
        img_feat = F.normalize(query_output.mean(dim=1), dim=-1)
        txt_feat = F.normalize(text_embeds[:,0,:], dim=-1)

        # layer_grad_check = self.Qformer.bert.encoder.layer
        # layer_grad_check.register_backward_hook(hook_function)
        ###=======MoCo=========###
        with torch.no_grad():
            self._momentum_update()
            query_output_m = self.encode_img_m(image, modality)
            ...
            #print('image_feat_m.shape', image_feat_m.shape)
            #print('self.image_queue.shape', self.image_queue.clone().detach().shape)
        ...
        with torch.no_grad():
            ...
            ###DIVISION###
            weights_i2i = F.softmax(sim_i2i[:, :bs], dim=1)
            weights_t2t = F.softmax(sim_t2t[:, :bs], dim=1)

            weights_i = torch.div(weights_i2t, weights_i2i)
            weights_t = torch.div(weights_t2i, weights_t2t)

            # if self.args.sampling == 'division':
            #     if epoch < self.config['schedular']['warmup_epochs']:
            #         weights_i = weights_i2t
            #         weights_t = weights_t2i
            #     else:
            #         weights_i2i = F.softmax(sim_i2i[:, :bs], dim=1)
            #         weights_t2t = F.softmax(sim_t2t[:, :bs], dim=1)
            ...
            # elif self.args.sampling == 'threshold':
            #     if epoch < self.config['schedular']['warmup_epochs']:
            #         weights_i = weights_i2t
            #         weights_t = weights_t2i
            #     else:
            #         for i in range(len(weights_i2t)):
            #             self_sim = weights_i2t[i][i]
            #             weights_i2t[i][weights_i2t[i] >= self_sim * 0.9] = 0.00000001
            #         weights_i = weights_i2t

            #         for i in range(len(weights_t2i)):
            #             self_sim = weights_t2i[i][i]
            #             weights_t2i[i][weights_t2i[i] >= self_sim * 0.9] = 0.00000001
            #         weights_t = weights_t2i

            # elif self.args.sampling == 'hard':
            #     weights_i = weights_i2t
            #     weights_t = weights_t2i
            ...
            weights_i.fill_diagonal_(0)
            weights_t.fill_diagonal_(0)

        # select a negative image for each text
        image_embeds_neg = []
        for b in range(bs):
            neg_idx = torch.multinomial(weights_t[b], 1).item()
        ...
        text_atts_neg = []
        for b in range(bs):
            neg_idx = torch.multinomial(weights_i[b], 1).item()

            text_embeds_neg.append(text_embeds[neg_idx])
            text_atts_neg.append(txt_tokens.attention_mask[neg_idx])

        text_embeds_neg = torch.stack(text_embeds_neg, dim=0)
        text_atts_neg = torch.stack(text_atts_neg, dim=0)

        text_embeds_all = torch.cat([text_embeds, text_embeds_neg], dim=0)
        text_atts_all = torch.cat([txt_tokens.attention_mask, text_atts_neg], dim=0)

        image_embeds_all = torch.cat([image_embeds_neg, image_embeds], dim=0)
        image_atts_all = torch.cat([image_atts, image_atts], dim=0)


        output_neg = self.Qformer.bert(encoder_embeds=text_embeds_all,
        ...
        #print('input embeds to the LLM', inputs_embeds.shape)
        with torch.autograd.detect_anomaly():
            ...
            llm_loss = 0.5*outputs.loss
            print('outputs.loss', llm_loss, 'loss_ita', loss_ita)
            loss = llm_loss + loss_ita
