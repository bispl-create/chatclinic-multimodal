"""Plugin entrypoint for the SegResNet brain tumor segmentation tool.

Thin wrapper around :mod:`.infer`: loads the tool's ``config.yaml``, injects
tool-local data/weights paths, applies payload overrides, and dispatches
to ``run_inference``. Returns the standard plugin-format dict.
"""
...
def _local_paths() -> Dict[str, str]:
    return {
        ...
        # infer.py reads bundle_dir/<bundle_name> and msd_data_dir/<dataset_task>
        "bundle_dir": str(_THIS_DIR / "weights"),
        "msd_data_dir": str(_THIS_DIR / "dataset"),
        "model_weights_path": str(_CHECKPOINT_DIR / "brain_tumor_seg.pt"),
    }
