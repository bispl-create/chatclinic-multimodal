...
class ModalityBatchSamplerGranular(BatchSampler):
    def __init__(self,
        ...
        for idx, info in enumerate(infos):
            ...
            # --- 여기서 modality 문자열 파싱 ---
            # 1) 옛날 버전: "Aortic enlargement in CXR"
            if ' in ' in cls_name:
                modality = cls_name.split(' in ')[-1].strip()

            # 2) 새 버전: "Chest X-ray showing an enlarged thoracic aorta."
            elif ' showing ' in cls_name:
                # "Chest X-ray showing ..." -> "Chest X-ray"
                modality = cls_name.split(' showing ')[0].strip()
