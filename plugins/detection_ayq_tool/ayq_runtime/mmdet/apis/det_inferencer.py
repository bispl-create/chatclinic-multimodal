...
class DetInferencer(BaseInferencer):
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        # Ensure target_class survives PackDetInputs -> data_sample metainfo.
        if pipeline_cfg[-1].get('type') == 'PackDetInputs':
            default_meta_keys = (
                'img_id', 'img_path', 'ori_shape', 'img_shape',
                'scale_factor', 'flip', 'flip_direction')
            meta_keys = list(pipeline_cfg[-1].get('meta_keys', default_meta_keys))
            meta_keys = [meta_key for meta_key in meta_keys if meta_key != 'img_id']
            if 'target_class' not in meta_keys:
                meta_keys.append('target_class')
            pipeline_cfg[-1]['meta_keys'] = tuple(meta_keys)
    ...
    def __call__(
            ...
            # custom embedding selector for AYQ models
            target_class: Optional[Union[str, list]] = None,
            **kwargs) -> dict:
        """Call the inferencer.
        ...
        if target_class is not None and isinstance(target_class, str):
            target_class = [target_class] * len(ori_inputs)

        # Currently only supports bs=1
        tokens_positive = [tokens_positive] * len(ori_inputs)

        if target_class is not None:
            assert len(target_class) == len(ori_inputs)
            for i in range(len(target_class)):
                if isinstance(ori_inputs[i], str):
                    ori_inputs[i] = {
                        'img_path': ori_inputs[i],
                        'target_class': target_class[i]
                    }
                elif isinstance(ori_inputs[i], np.ndarray):
                    ori_inputs[i] = {'img': ori_inputs[i], 'target_class': target_class[i]}
                else:
                    ori_inputs[i]['target_class'] = target_class[i]

        if texts is not None:
            assert len(texts) == len(ori_inputs)
            for i in range(len(texts)):
                if isinstance(ori_inputs[i], dict):
                    ori_inputs[i].update({
                        'text': texts[i],
