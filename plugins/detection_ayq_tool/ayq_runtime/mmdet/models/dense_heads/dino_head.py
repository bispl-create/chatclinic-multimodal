...
# MMEngine이 기대하는 형태로 반환
# 방법 1: BaseDataElement 형태로 반환
...
@MODELS.register_module()
class DINOHead_ContrastiveMOCA(DeformableDETRHead):
    r"""Head of the DINO: DETR with Improved DeNoising Anchor Boxes
    for End-to-End Object Detection
...
@MODELS.register_module()
class DINOHead_ContrastiveMOCA_QM(DeformableDETRHead):
    r"""Head of the DINO: DETR with Improved DeNoising Anchor Boxes
    for End-to-End Object Detection
    ...
    def __init__(self,
        ...
        # num_pred_layer를 kwargs에 명시적으로 추가
        kwargs['num_pred_layer'] = num_pred_layer
...
@MODELS.register_module()
class DINOHead_ContrastiveMOCA_QM_OnlyCL(DeformableDETRHead):
    r"""Head of the DINO: DETR with Improved DeNoising Anchor Boxes
    for End-to-End Object Detection
        ...
        if contrastive_features is not None and contrastive_clip_text_features is not None:
            ...
            for i, query_feat in enumerate(contrastive_features):
                if i in self.contrastive_apply_layers or (i - len(contrastive_features)) in self.contrastive_apply_layers:
                    ...
                    # 예: i == -1 (마지막 레이어) 지원 위해 음수 index도 허용
                    #print('USING LAYER FOR CONTRASTIVE: ', i)
                    pooled = query_feat[:,num_denoising_queries:,:].mean(dim=1, keepdim=True)
                    projected_query = self.query_mlp(pooled)
                    contrastive_loss = self.contrastive_loss_fn(projected_query, text_feat)
                    loss_dict[f'loss_contrastive_layer{i}'] = contrastive_loss
    ...
    def loss_by_feat_predict(
        ...
        if contrastive_features is not None and contrastive_clip_text_features is not None:
            ...
            text_feat = torch.cat(
                [
                    torch.from_numpy(feat[1]).unsqueeze(0).to(device) if isinstance(feat[1], np.ndarray) else
                    feat[1].unsqueeze(0).to(device) if feat[1].ndim == 2 else feat[1]
                    for feat in contrastive_clip_text_features
                ],
                dim=0
            )  # [bs, dim]
            ...
            save_dir = '/path/to/repository/contrastive_query_feats_afterQREPA_Det_layer0'
            os.makedirs(save_dir, exist_ok=True)

            for i, query_feat in enumerate(contrastive_features):
                if i == 0:#:in self.contrastive_apply_layers or (i - len(contrastive_features)) in self.contrastive_apply_layers:
                    
                    # 예: i == -1 (마지막 레이어) 지원 위해 음수 index도 허용
                    pooled = query_feat.mean(dim=1, keepdim=True)
                    projected_query = self.query_mlp(pooled)
                    # modified for saving npy features  
                    contrastive_loss, query_feat = self.contrastive_loss_fn(projected_query, text_feat)
                    val_loss_dict[f'val_loss_contrastive_layer{i}'] = contrastive_loss

                    # 🔥 여기서 query_feat_out 저장
                    with torch.no_grad():
                        # (bs, 1, dim) 또는 (bs, dim) 형태일 수 있으니 정리
                        q = query_feat.detach().cpu()
                        if q.dim() == 3:
                            q = q.squeeze(1)  # (bs, dim)

                        # 배치 차원 기준으로 파일 이름과 매칭해서 저장
                        for b, (meta, q_vec) in enumerate(zip(contrastive_clip_text_features, q)):
                            data_name = meta[0]  # contrastive_clip_text_features[b][0] : 파일 이름
                            save_path = os.path.join(save_dir, f"{data_name}.npy")
                            np.save(save_path, q_vec.numpy())
...
# Use All Loss
@MODELS.register_module()
class DINOHead_ContrastiveMOCA_QM_AL(DeformableDETRHead):
    r"""Head of the DINO: DETR with Improved DeNoising Anchor Boxes
    for End-to-End Object Detection
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
             ...
             dn_meta: Dict[str, int],
             contrastive_features: Optional[List[Tensor]] = None,
             contrastive_clip_text_features: Optional[List[Tensor]] = None) -> Dict[str, Tensor]:
        """Perform forward propagation and loss calculation of the detection
        head on the queries of the upstream network.
    ...
    def loss_by_feat(
        ...
        """ use all loss + compute contrastive loss."""
        ...
        # 이전의 것들과 함께 contrastive loss 부분 추가
        if contrastive_features is not None and contrastive_clip_text_features is not None:
