...
from ..layers import (DeformableDetrTransformerDecoder,
                      DeformableDetrTransformerDecoder_ContrastiveMOCA,
...
class DeformableDETR_ContrastiveMOCA(DetectionTransformer):
    ...
    def _init_layers(self) -> None:
        ...
        self.decoder = DeformableDetrTransformerDecoder_ContrastiveMOCA(**self.decoder)
    ...
    def forward(self,
        ...
        if mode == 'loss':
            ...
            if self.training:
                embedding_folder_path = '/path/to/repository/text_embedding_maker_modality_pubmedclip/instances_train2017_revised'

            # loss를 수정하면서 else 부분과 같은 상황이 발생
            else:
                embedding_folder_path = '/path/to/repository/text_embedding_maker_modality_pubmedclip/instances_val2017_revised'
            # embedding_folder_path = '/path/to/repository/text_embedding_maker_modality/instances_train2017_revised'
    ...
    @torch.no_grad()
    def val_step(self, data_batch: dict) -> list:
        # 1) preprocessor
        data_batch = self.data_preprocessor(data_batch, training=False)
        imgs = data_batch['inputs']
        samp = data_batch['data_samples']

        # 2) loss 모드로 forward
        losses = self.forward(imgs, samp, mode='loss')

        # 3) BaseDataElement에 losses 를 담아서 list로 리턴
        loss_data = BaseDataElement()
        loss_data.loss = losses
        # print('loss_data: ', loss_data)

        # outputs[-1] 이 BaseDataElement( keys()==['loss'] ) 가 되도록
        return [loss_data]
