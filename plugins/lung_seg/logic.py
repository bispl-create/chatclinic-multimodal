"""Plugin entrypoint for the PSPNet CXR lung segmentation tool."""
...
def _resolve_input_path(payload: Dict[str, Any]) -> str:
    for key in ("source_image_path", "image_path", "nifti_path", "dicom_path", "input_path"):
        raw = payload.get(key)
        if raw:
            return str(raw).strip()
    raise ValueError(
        "Payload must include one of: source_image_path, image_path, nifti_path, dicom_path, input_path."
    )
...
def _build_cfg(payload: Dict[str, Any], input_path: str):
    ...
    cfg.paths.cxr_image_path = input_path
...
def execute(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = (
        f"{cfg.tool.display_name}: segmented {num_samples} image(s) from "
